...
var $author$project$Main$houseHeight = 4;
var $author$project$Main$houseWidth = 3;
var $author$project$Main$windowsCount = $author$project$Main$houseWidth * $author$project$Main$houseHeight;
var $author$project$Main$init = A2($elm$core$Array$repeat, $author$project$Main$windowsCount, $author$project$Main$Dark);
...
var $author$project$Main$Seedlings = {$: 'Seedlings'};
...
var $author$project$Main$update = F2(
	function (msg, model) {
		var clickWindow = A2($elm$core$Array$get, msg, model);
		if (clickWindow.$ === 'Just') {
			switch (clickWindow.a.$) {
				case 'Dark':
					var _v1 = clickWindow.a;
					return A3($elm$core$Array$set, msg, $author$project$Main$Light, model);
				case 'Light':
					var _v2 = clickWindow.a;
					return A3($elm$core$Array$set, msg, $author$project$Main$Seedlings, model);
				default:
					var _v3 = clickWindow.a;
					return A3($elm$core$Array$set, msg, $author$project$Main$Dark, model);
			}
		} else {
			return model;
		}
...
var $timjs$elm_collage$Collage$Layout$vertical = A2(
	...
	$timjs$elm_collage$Collage$Layout$empty);
var $author$project$Main$house = function (model) {
	var wall = A2(
		$timjs$elm_collage$Collage$filled,
		$timjs$elm_collage$Collage$uniform($avh4$elm_color$Color$charcoal),
		A2($timjs$elm_collage$Collage$rectangle, $author$project$Main$houseWidth, $author$project$Main$houseHeight));
	var vspace = A2($timjs$elm_collage$Collage$Layout$spacer, 0, 0.3);
	var split = F2(
		function (n, list) {
	...
	var hspace = A2($timjs$elm_collage$Collage$Layout$spacer, 0.3, 0);
	var makeFloorLayout = function (floor) {
		return $timjs$elm_collage$Collage$Layout$horizontal(
			A2($elm$core$List$intersperse, hspace, floor));
	};
	var fillWindow = F2(
		function (id, window) {
			var drawWindowColoredIn = function (fillStyle) {
				return A2(
					...
					A2(
						$timjs$elm_collage$Collage$filled,
						fillStyle,
						$timjs$elm_collage$Collage$square(0.5)));
			};
			switch (window.$) {
				case 'Dark':
					return drawWindowColoredIn(
						$timjs$elm_collage$Collage$uniform($avh4$elm_color$Color$darkCharcoal));
				case 'Light':
					return drawWindowColoredIn(
						$timjs$elm_collage$Collage$uniform($avh4$elm_color$Color$lightYellow));
				default:
					return drawWindowColoredIn(
						$timjs$elm_collage$Collage$uniform($avh4$elm_color$Color$lightPurple));
			}
		});
	var windows = A2(
	...
	var windowsByFloors = A2(split, $author$project$Main$houseWidth, windows);
	var floors = A2($elm$core$List$map, makeFloorLayout, windowsByFloors);
		...
		_List_fromArray(
			[
				...
				$timjs$elm_collage$Collage$Layout$vertical(
					_List_fromArray(
						[
							A2($timjs$elm_collage$Collage$Layout$spacer, 0, 0.1),
							$timjs$elm_collage$Collage$Layout$stack(
							_List_fromArray(
								[
									$timjs$elm_collage$Collage$Layout$center(
									$timjs$elm_collage$Collage$Layout$vertical(
										A2($elm$core$List$intersperse, vspace, floors))),
									wall
								]))
						])))
			]));
			...
			[
				...
				_List_fromArray(
					[
						...
						$author$project$Main$house(model))
					]))
			]));
};
